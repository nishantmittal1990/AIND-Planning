...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # for this action node to be reachable in planning graph. It's preconditions must be
            # Satisfied by previous state level.
            if node_a.prenodes.issubset(self.s_levels[level]):
                # Add this reachable action node to list of action nodes.
                a_nodes.append(node_a)
                # now, make this reachable action node to child of previous level state node
                # also make that state node as a parent of child action node
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    node_a.parents.add(node_s)
        # Now, build action level from reachable action nodes.
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            # We need to get effect nodes of previous action level and the same to level's state node.
            # As describe above - every A node in the previous level has a list of S nodes in effnodes that represent the effect
            # produced by the action.  These literals will all be part of the new S level
            for node_s in node_a.effnodes:
                # Add this effect node in set of state nodes at LEVEL
                s_node.add(node_s)
                # Add action node at level-1 as parent of state node.
                node_s.parents.add(node_a)
                # Add state node at level as a child of previous level action node.
                node_a.children.add(node_s)
        # Build state level from reachable effect nodes.
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # node_a1 pos effects are not cancelled by node_a2 neg effects
        for effects in node_a1.action.effect_add:
            # check if positive effect of action a1 is cancelled by negative effect of action a2
            if effects in node_a2.action.effect_rem:
                # then return True
                return True

        # node_a2 pos effects are not cancelled by node_a1 neg effects
        for effects in node_a2.action.effect_add:
            # check if positive effect to acton a2 is cancelled by negative effect of action a1
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check for action a1 positive effects doesn't conflict with negative pre-condition of action a2
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True
        # Check for action a2 positive effects doesn't conflict with negative pre-condition of action a1
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # Check for action a1 negative effects doesn't conflict with positive pre-condition of action a2
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True
        # Check for action a2 negative effects doesn't conflict with positive pre-condition of action a1
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # check if a1 pre-conditions are not mutually exclusive with a2 pre-conditions
